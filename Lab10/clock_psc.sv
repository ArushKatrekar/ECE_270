...
module ll_memory #(parameter ALTITUDE = 16'h4500,parameter VELOCITY = 16'h0,parameter FUEL = 16'h800,parameter THRUST = 16'h5)(
...
module ll_control(input  logic clk,rst,input  logic [15:0]alt, vel, output logic land, crash,wen);

  logic [15:0]alt_next;
  bcdadd4 lookahead(.a(alt),.b(vel),.ci(1'b0),.s(alt_next),.co());

  logic reached_ground;
  assign reached_ground = (alt == 16'h0000)||(alt_next == 16'h0000)||(alt_next[15:12] == 4'd9);
  ...
  assign going_too_fast = (vel[15:12] == 4'd9) && !vel_ge_neg30;
  always_ff @(posedge clk or posedge rst) begin
    if (rst) begin
      land<= 1'b0;
      crash<= 1'b0;
...
module lunarlander #(
  ...
  // lim=3: toggles every 4 posedge hz100, matching testbench hzX rate
  clock_psc psc(
    .clk(hz100),
    .rst(reset),
    .lim(8'd55),
